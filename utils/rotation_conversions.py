...
def random_rotation_saul(
    ...
    constrained within the provided limits.
    
    Args:
        coords: Tensor of shape (N, 3) representing 3D coordinates.
        feats: Tensor of shape (N, 1) representing the features.
        angle_limits: Tensor of shape (2, 3) containing the min and max limits for each axis.
                      First row is the minimum angle, and the second row is the maximum angle.
                      Angles should be in radians.
        origin: Tensor of shape (3,) representing the new origin for the rotation.
        convention: String of three uppercase letters representing the rotation axes. 
                    E.g., "XYZ" means rotating first around X, then Y, then Z. 
                    The default is "XYZ".
    
    Returns:
        Rotated coordinates as a tensor of shape (N, 3).
    """
    if angle_limits.shape != (2, 3):
        raise ValueError(f"Expected `angle_limits` to have shape (2, 3), but got {angle_limits.shape}")
    if origin.shape != (3,):
        raise ValueError(f"Expected `origin` to have shape (3,), but got {origin.shape}")

    # Generate random angles within the provided limits
    min_angles, max_angles = angle_limits[0], angle_limits[1]
    angles = min_angles + (torch.rand(3) * (max_angles - min_angles))

    # Translate coords to align the origin at (0, 0, 0)
    translated_coords = coords - origin

    # Compute the rotation matrix using the constrained random Euler angles
    ...
    rotated_coords = translated_coords @ rotation_matrix.numpy() 
