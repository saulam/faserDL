...
def ini_argparse(
    ...
    parser.add_argument('-ws', '--warmup_epochs', type=int, default=0, help='number of warmup epochs')
    parser.add_argument('--cosine_annealing_epochs', type=int, default=0, help='number of cosine annealing epochs')
    ...
    parser.add_argument("--head_dropout_cls", type=float, default=0.0, help="Dropout rate for classification/vertex heads (overfit-prone tasks)")
    parser.add_argument("--head_dropout_reg", type=float, default=0.0, help="Dropout rate for regression heads (momentum tasks)")
    parser.add_argument("--cls_lr_scale", type=float, default=1.0, help="LR multiplier for classification/vertex heads (e.g. 0.5 to slow convergence of overfit-prone tasks). Applied on top of layer-wise LR decay.")
    parser.add_argument("--kendall_w_max", type=float, default=5.0, help="Upper bound of the Kendall uncertainty weight w=w_min+(w_max-w_min)*sigmoid(u). Lower values (e.g. 2.0) prevent any single task from dominating the gradient.")
    ...
    parser.add_argument("--save_dir", type=str, default=".", help="base directory for logs")
