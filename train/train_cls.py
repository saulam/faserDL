...
from utils import CustomFinetuningReversed, ini_argparse, split_dataset, supervised_pixel_contrastive_loss
from dataset import SparseFASERCALDatasetCls
from model import MinkClsConvNeXtV2, SparseClsLightningModel
...
def main():
    ...
    # Dataset
    dataset = SparseFASERCALDatasetCls(args)
    ...
    model = MinkClsConvNeXtV2(in_channels=1, out_channels=4, D=3, args=args)
    #print(model)
    ...
    # Lightning model
    lightning_model = SparseClsLightningModel(model=model,
        loss_fn=loss_fn,
        args=args)
