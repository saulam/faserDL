...
from .utils import GlobalFeatureEncoder, Block, MinkowskiLayerNorm, MinkowskiCrossAttention
...
class MinkUNetConvNeXtV2(nn.Module):
    def __init__(self, in_channels, out_channels, D=3, args=None):
        ...
        for i in range(self.nb_elayers):
            ...
            # SE layer
            se_layer = MinkowskiCrossAttention(in_channels=encoder_dims[i], glob_dim=encoder_dims[0])#, reduction=se_block_reds[i])
        ...
        #    MinkowskiGlobalAvgPooling(),
        #    MinkowskiLayerNorm(encoder_dims[i], eps=1e-6),
        #    MinkowskiLinear(encoder_dims[i], 4),
        #)
    ...
    def forward(self, x, x_glob):
        ...
        #out_flavour = self.flavour_head(x)
        ...
        return {#"out_flavour": out_flavour,
