"""Lightning module for scintillator PID transfer."""
...
from .scintillator_dataset import CLASS_NAMES, NUM_CLASSES


class ScintillatorPIDFineTuner(pl.LightningModule):
    """Lightning wrapper for scintillator PID transfer."""
    ...
    def forward(self, x_sp, global_features=None):
        return self.model(x_sp, global_features)

    def training_step(self, batch, batch_idx):
        x_sp = self._build_sparse(batch)
        outputs = self(x_sp, batch["global_features"])
        loss = self.loss_fn(outputs["logits"], batch["label"])

        B = batch["batch_size"]
        preds = outputs["logits"].argmax(dim=1)
        self.train_acc.update(preds, batch["label"])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_acc_per_class.update(preds, batch["label"])
    ...
    def configure_optimizers(self):
        ...
        if self.trainer.is_global_zero:
            print(
                f"lr={self.lr:.6f}  total_steps={total_steps}  "
                f"warmup={warmup_steps}  cosine={cosine_steps}"
            )
    ...
    def _build_param_groups(self):
        """Build parameter groups for layer-wise LR decay."""
        ...
        for name, p in self.model.named_parameters():
            ...
            elif name.startswith("patch_embed") or name.startswith("cls_token"):
