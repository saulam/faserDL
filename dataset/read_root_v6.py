...
output_dir = '/scratch/salonso/sparse-nns/faser/events_v{}b'.format(version)
os.makedirs(output_dir, exist_ok=True)
ROOT.gSystem.Load("/scratch5/FASER/V5.x_27102025/FASER/Python_io/lib/ClassesDict.so")

# Placeholder for class objects
tcal_event = ROOT.TcalEvent()
tporeco_event = ROOT.TPORecoEvent()
MAXMUTRACKS = 10
ms_ntracks = array('i', [0])
ms_charge  = array('f', [0.]*MAXMUTRACKS)
ms_npoints = array('i', [0]*MAXMUTRACKS)
ms_px      = array('f', [0.]*MAXMUTRACKS)
ms_py      = array('f', [0.]*MAXMUTRACKS)
ms_pz      = array('f', [0.]*MAXMUTRACKS)
ms_p       = array('f', [0.]*MAXMUTRACKS)
ms_chi2    = array('f', [0.]*MAXMUTRACKS)
ms_nDoF    = array('i', [0]*MAXMUTRACKS)
ms_pval    = array('f', [0.]*MAXMUTRACKS)
ms_fpErr   = array('f', [0.]*MAXMUTRACKS)
ms_fipErr  = array('f', [0.]*MAXMUTRACKS)
...
def get_muon_spectrometer():
    ntracks = min(int(ms_ntracks[0]), MAXMUTRACKS)
    mu_charge  = np.array(ms_charge[:ntracks],  dtype=np.float32)
    mu_npoints = np.array(ms_npoints[:ntracks], dtype=np.int32)
    mu_px      = np.array(ms_px[:ntracks],      dtype=np.float32)
    mu_py      = np.array(ms_py[:ntracks],      dtype=np.float32)
    mu_pz      = np.array(ms_pz[:ntracks],      dtype=np.float32)
    mu_p       = np.array(ms_p[:ntracks],       dtype=np.float32)
    mu_chi2    = np.array(ms_chi2[:ntracks],    dtype=np.float32)
    mu_nDoF    = np.array(ms_nDoF[:ntracks],    dtype=np.int32)
    mu_pval    = np.array(ms_pval[:ntracks],    dtype=np.float32)
    mu_fpErr   = np.array(ms_fpErr[:ntracks],   dtype=np.float32)
    mu_fipErr  = np.array(ms_fipErr[:ntracks],  dtype=np.float32)
    rows = np.array([mu_charge, mu_npoints, mu_px, mu_py, mu_pz, mu_p,
                     mu_chi2, mu_nDoF, mu_pval, mu_fpErr, mu_fipErr])
    return ntracks, rows
...
def generate_events(number, chunks, disable):
    ...
    for i, reco_file_path in t:
        ...
        ms_tree   = reco_file["MuonSpectrometer"]

        # disable problematic fMuTracks!
        reco_tree.SetBranchStatus("TPORecoEvent.fMuTracks", 0)
        reco_tree.SetBranchAddress("TPORecoEvent", tporeco_event)

        #reco_tree.Print()
        total_entries = reco_tree.GetEntries()
        reco_tree.SetBranchAddress("TPORecoEvent", tporeco_event)
        ms_tree.SetBranchAddress("ntracks", ms_ntracks)
        ms_tree.SetBranchAddress("charge",  ms_charge)
        ms_tree.SetBranchAddress("npoints", ms_npoints)
        ms_tree.SetBranchAddress("px",      ms_px)
        ms_tree.SetBranchAddress("py",      ms_py)
        ms_tree.SetBranchAddress("pz",      ms_pz)
        ms_tree.SetBranchAddress("p",       ms_p)
        ms_tree.SetBranchAddress("chi2",    ms_chi2)
        ms_tree.SetBranchAddress("nDoF",    ms_nDoF)
        ms_tree.SetBranchAddress("pval",    ms_pval)
        ms_tree.SetBranchAddress("fpErr",   ms_fpErr)
        ms_tree.SetBranchAddress("fipErr",  ms_fipErr)
    
        for entry_idx in range(total_entries):
            reco_tree.GetEntry(entry_idx)
            ms_tree.GetEntry(entry_idx)
            ...
            # not process nutaus
            if not (is_cc and abs(in_neutrino_pdg) == 16):
            ...
            # Muon spectrometer
            muspec_ntracks, muspec_info = get_muon_spectrometer()
